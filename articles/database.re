...
#@# gfx: GitHubの contentHTML とかはクライアントサイドで計算しようがないのでREST APIでもレスポンスにいれるしかないよ。まあ、その計算が重いとREST APIの微妙に仕様の違うパスを大量に定義するはめになるわけだけど。
...
#@# wawoon: GraphQLでは事前にどんなクエリが来るのかわからないので、多段にinner joinしたeager loadingで事前にレコード取得ができないというのがわかるといいと思います。
...
#@# gfx: トラブルが発生してから対応するのはたしかに面倒だけど、GraphQLだと「あとからでも対応ができる」というのは大きな強みだと思う。REST APIだとAPI自体再設計してクライアントも修正して…みたいになってしまいがち。実際、問題にならない程度の個数のN+1の対応は後回しでいいと思うのよね。
...
#@# wawoon: 分散DB（というかKVS）がGraphQLの実装では相性がいいことをもう少し書いてもいいかなと思いました。
#@# wawoon: 親のノードは大抵の場合子ノードのIDを知っているので高機能なクエリ言語は不要で、スケーラビリティの高いDBのほうが良いなど。
