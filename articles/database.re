...
何回か書いている気もしますが、筆者はGoogle Cloud Datastoreという分散KVS的なものを10年近く使い続けています。
...
GraphQLのスキーマでは色々な型と、それぞれにフィールドがあります。
...
2つ目の別の型やリストへの参照はスカラ型以外の、オブジェクトやインタフェースで表現され、そのフィールドにさらに何らかの値を持ちます。
3つ目はDB上に具体的な値として持っていないか、加工が必要になるものです。

1つ目のスカラ型の値はDBに保存されているはずなので、そのまま直接翻訳することができます。
...
N+1問題@<fn>{n+1-not-in-wikipedia}はDB次第、特にRDBでは意識しておかないと障害として顕在化しやすいです。
...
爆発ですね。
セキュリティ対策としてのcomplexityの話は@<chapref>{github}で言及しています。
...
筆者はGoogle Cloud Datastore用のラッパライブラリを作っていて、それに処理のバッチ化の機能@<fn>{mercari-datastore-batch}を持たせてそれを使っています。
...
解決方法としては、実装を諦めるとよいです。
自動読み込みによる無限スクロールとかがお勧めです。
これはマジです。
データ量が大量になってしまいがちな現代ではページ数よりも検索でカバーしたほうが無難な設計かもしれません。
