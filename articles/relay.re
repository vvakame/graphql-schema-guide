...
さて、返すデータが何かですが、この例からも分かるとおり、削除したデータ自体を返しています。
これはもうデータベース上に存在しないデータを返していることになるため、もしかしたらレスポンスに使うresolverの実装次第ではエラーになってしまう可能性があります。
第2版執筆時点では、エラーになるシチュエーションには出会っていません。
これはデータ自体はデータベースに存在しなくても、resolverが消したデータをIDを使って再参照するようなシチュエーションはそうそう発生しないからだと考えられます。

GitHubがどうしているか、リアクションを削除する@<code>{removeReaction}のMutationの返り値を見てみました。
すると、消されたリアクションそのものである@<code>{reaction: Reaction}と、親子関係があったであろう@<code>{subject: Reactable}の2つを返しています。
消されたデータ本体だけではなく、その親も返すのはキャッシュ更新の観点では便利そうですね。
