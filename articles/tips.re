...
#@# sonatard: これ以降のファイル分割の具体例がどういう方針でファイル分割しようとしているかが最初にわかるとコードが読みやすいと思いました。これがないとGraphQLのファイル分割のために必要な作業の説明か、ファイル分割の方針の説明かの区別が難しかったです。
...
#@# sonatard: これにより「Queryの時点で画面の描画に必要ではない値まで取得する必要が生じます。それはGraphQLのよさを犠牲にしてしまいます。」が解決される理由がわかりませんでした。そういう話ではない...？
