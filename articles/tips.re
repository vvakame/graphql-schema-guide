...
TODO もうちょっとよい章タイトルにならんですか。

この章では、他の章に分類できなかったけど書いたおきたいことや挫折していることを紹介していきます。

=={frontend-matter} 画面の都合とGraphQL

GraphQLのクエリは非常に柔軟な表現力を持ちます。
ゆえにサーバ側の助けを借りずに自分にとって必要なデータをきれいに取得できます。
クライアント側とサーバ側の実装の詳細を相互に気にする必要がなくなるので大変便利です。
...
クライアント側で考えたり吸収すべき問題が多くなりすぎですね。

これを解決するため、@<code>{batchCreateCircleExhibitInfoAndProductInfos}のようなAPIを作ってもよいでしょう。
美的観点で見るとヤバいと思うと思いますが、クライアント側からすると1回のアクセスで完結するので便利です。

GraphQLのspecとして、Mutationのある部分の返り値を動的な変数として別の場所で引数として使える、という仕様@<fn>{spec-dynamic-var}を提案している人がいます。
クライアント側からすると少しうれしいかもしれませんが、サーバ側のフレームワーク開発者から見ると地獄みがあります。
...
というわけで、スキーマを分割して複数のファイルで定義したくなります。
GraphQLはスキーマを分割して定義するための仕様を持っています。
それが@<code>{extend}です。
この仕様を活用すると、@<code>{Query}型の定義を複数箇所に分割して行えたりするわけです。

あとは、複数のファイルを適当にconcatすれば1つのスキーマファイルが手に入ります。
大抵のツールは複数のファイルをいい感じにマージして処理する機能がついていることでしょう。
...
さらに既存の型を拡張していきます（@<list>{code/tips/schemaExtend/circleExhibitInfo.graphql}）。
DBのスキーマ上、@<code>{Event}と@<code>{CircleExhibitInfo}の紐付けは子となる側、この場合は@<code>{CircleExhibitInfo}が保持しています。
GraphQLスキーマ上での定義を行う箇所も、これに倣います。
...
# DBスキーマ上、EventはサークルのIDを持っているわけではない
# データとしての主体はCircleExhibitInfo側なので、Eventをここで拡張する
extend type Event {
  # Relay仕様は紙面の都合で無視
...
であれば、子側となる@<code>{CircleExhibitInfo}に親との関係性が記述してあるのは自然であり、管理もより容易くなるでしょう。
...
Prismaのimport syntaxの提案@<fn>{prisma-import}があるのですが、あまり受け入れられ、流行ってる感じはしません。
...
//list[error-extensions][エラーの書式]{
{
  "errors": [
    {
      "message": "status 400: E4-0009 - bad request. detail: invalid id syntax: notExists",
      "path": [
        "updateCheckedCircle"
      ],
      "extensions": {
...
それはドメイン層の権限チェックの結果であったり、スキーマ上からは汲み取れない裏側のフラグや、ユーザや条件の組み合わせに依存していたりしました。

GraphQLにおいては、予測可能な結果を返さないのは罪です。
スキーマから汲み取れないことは行ってはいけません。
クエリがスキーマに対して正しいのであれば、必ず結果を返すか、さもなくばわかりやすい、見ただけで問題を解決可能なエラーを返しましょう。
...
一般ユーザは何をしてはいけないのか？使ってよいパラーメタはどれか？
これを理解するのが難しく、使い物になりませんでした。
...
ここでエラーを見て分かるのはフロントエンドの開発者であり、エンドユーザである必要はないと考えています。
...
ドメイン層のロジックと、GraphQLのレイヤーの噛み合わせは必ずしも楽に進むものではありません。

筆者の使っているGo言語+Cloud Datastoreの構成では、stringをDBに保存する時、NULLにするという概念が薄いです。
値が空ならば、空文字列が保存されます。
この考え方はGraphQLの世界観とは相性が悪いです。
そこで、Go言語+DBの慣習に従う世界観とGraphQL世界観に従う層をどこかで明確に線引きするのがよいでしょう。

ここではスキーマファーストのやり方のほうが都合がよいです。
...
GraphQLスキーマ的には更新したい箇所の値だけ渡せばよい構造にしたいころです。
もし、更新しない値もすべて要求するのであれば、Queryの時点で画面の描画に必要ではない値まで取得する必要が生じてしまうかもしれません。
それはGraphQLのよさをいくらかスポイルしてしまいます。
...
GraphQL以外のAPIを提供するとき、GraphQLと同様のチェック機構を後から組むこむのは辛いです。
よって、最初からドメイン層に統一的に組み込んでおくとよいでしょう。
...
筆者も稀にresolver内にDBへのクエリを直接書いてしまう場合があります。
ドメイン層に該当の処理を書くのが面倒でやってしまう場合があるのですが、本来であれば自省するべきです…。
