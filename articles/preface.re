...
#@# gfx: 「スキーマファースト」よりも「スキーマドリブン（スキーマ駆動）」のほうが大抵の場合は実態に即している気がして好き。ただこれだと「コードファースト」に対応する一般的な語がないから「スキーマ駆動をするかどうかに限らず」みたいになるけど。まあ、好みです！（が、個人的にはこだわりがあるのでどうしても言いたかった）
...
#@# gfx: "Graph構造のデータから" → ほんとかな！？https://graphql.org/ のトップにもgraph構造には触れてないよ！バックエンドのデータもRDBMS（表）だったりKVSだったりするよ！ただGraphQLの戻り値はツリー構造で、ツリー構造はグラフ構造の一種なので（表を扱うSQLとの比較でいうと）グラフ構造と無関係ではないものの。シンプルに説明するのは難しいね！
