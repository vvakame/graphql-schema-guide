...
GraphQLの基礎はスキーマであり、どう設計するかでクライアントがどの程度楽をできるか、サーバ側で制約をかけやすくなるかなどが決まります。
...
@<chapref>{tbf-ticket}は単なるオマケです。技術書典公式Webでの機能開発の日々を日記でお送りします。
...
さらに、クエリからどういう構造+型のデータが得られるか分かるので、フロントエンド側でデータに対して正しい使い方をしているかを静的に検査できます。
最高ですね！
...
さらに@<code>{User}から@<code>{FriendIDs}を取得することはできそうですし、そうしたら先のresolverを利用し@<code>{User}の配列にすることもできそうです。
...
もちろん@<code>{User.CPU使って計算するフィールド}のような、本来DB上に存在しないものには個別にresolverの実装を与える必要があります。
