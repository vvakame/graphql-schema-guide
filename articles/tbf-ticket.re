...
技術書典Webで一般入場整理券販売の機能を追加開発を行う記録を残していきます@<fn>{rev2-note}。
別の言い方をすると、一般入場整理券の機能をこれからでっち上げないといけない事実があり、ドタバタとそれを開発するログを綴っていきます。

//footnote[rev2-note][第二版注：この章は第1版で書いたものであり、すべての日付は2019年のものです。]

なお、技術書典7は9月22日に開催されます。

==[notoc] 8月27日（火） 日記を書き始める
...
//footnote[sheep1][ひつじ注：このときのわかめは「うううううーーーーーん」ってかんじだった]
...
とりあえず疲れて曖昧な気持ちでスキーマ書いてても自分の作ったlintルールが一貫性がぶっ壊れているところを指摘してくれる。
怒られたら直す。
平和守られる。
みんなもlintルール書け！
...
==[notoc] 9月2日（月）テストを書くとバグが見つかるんだな
...
販売システムと当日運用システムはまた別なので悩ましいところだ@<fn>{sheep2}。
まずは販売だけできるようにしなければ…。

//footnote[sheep2][ひつじ注：当日運用はそれはそれで大変なので人間が介在するととにかく要件が増えるのだ]
...
Stripeの動作確認は明日にしよう…。
それにしても課金系の動作確認は本当にめんどくさい…@<fn>{sheep3}。

//footnote[sheep3][ひつじ注：ぼくらがわるくなくても決済システムの障害とかに巻き込まれるので外部依存するとそれなりに大変]

とりあえずAPIはmergeして適当にリリースしてみる。
しかし、現状のままだと販売待ち状態がないのでUIのリリースは販売開始とイコールになってしまう…。
...
==[notoc] 9月4日（水）アプリ側対応の話

関係者各位に仕様を説明して開発をお願いするなどしてきた。
実に2週間ちょい前なのである…！

一般参加者用の整理券処理システム、サークル入場用と違ってそもそもの発行枚数が多い。
サークル用システムはアプリがオフラインキャッシュを作るのすら時間がかかって大変そうなので、別方式を考える必要がありそうだ。
...
僕はもう本当にeメールの運用というものを憎んでいて、使いたくないんだけど仕方ない…@<fn>{sheep5}。
...
//footnote[sheep5][ひつじ注：プッシュ通知ほしい。プッシュ通知ほしい。大事なことなので２回いいました]
...
==[notoc] 9月13日（金）end of the week

11日、12日の日記は後で読み返したらマジでただの作業ログだったので闇に葬りました@<fn>{sheep6}。
歯抜けの日があったらそういうことだと思ってください…。

//footnote[sheep6][ひつじ注：読んでみたけど差がわからなかったので読みたい人はわかめに励ましのお便りをおくろう！]
...
表紙を作るためにゆかりさんにかわいい表情して！って頼んでるんだけどおすましフェイスしててなかなか激プリティ写真が撮れない…@<fn>{sheep7}。

//footnote[sheep7][ひつじ注：毎日撮ってるので結構うまくなったってばっちゃが言ってた]

==[notoc] 9月15日（日）本番環境のマイグレーション作業
...
たまには運動は必要@<fn>{sheep8}なんやなって…。

//footnote[sheep8][ひつじ注：わかめは食生活も悪い]
...
//footnote[sheep9][ひつじ注：ぎじゅつしょてんのタスクが仕事だったらここまでまじめにやらないので休みにやるのはただしい気がする。でも休みはほしい]

==[notoc] 9月17日（火）ブログを書くのは大変
...
今日が見本誌の事前提出期限なので、いったん今日で終わりということにする。
羊に日記みたい？と聞いたら、「訳注みたいな感じで脚注色々入れて遊んでいいなら見る」というのでリポジトリのアクセス権限、くれてやるぜ！！

コンタクトレンズの在庫が期限expireしてたので出社前にコンタクトレンズ新調してもらってきた。
2年くらい前に比べて、微妙に目が悪くなってるっぽい。
がんばってほしい。
