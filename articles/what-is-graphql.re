...
//footnote[component-state][持つとしたら、大本のデータ更新の準備をするためのトランザクション的状態]

アプリケーションがたった1つの状態データ（single source of truth）を参照し、各コンポーネントはこれを分担してレンダリングすると考えます。
宣言的UIを実現するためのフレームワークを使っているのであれば、状態に従ってUIをレンダリングすれば画面は表示されます。
...
よって、良いスキーマというのは状態データへのパッチ作業、言い換えるとキャッシュ内のデータ操作が不足なくできるものを指します。
