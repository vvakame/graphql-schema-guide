...
DBの@<code>{User}テーブルから@<code>{friendIDs}を取得することもできそうです。
...
本書を最後まで読んで、または実際にGraphQLを使ったシステムを組んでみて、その後でもう一度読み返してみてください。

GraphQLというのはフロントエンドやアプリケーションのための技術です@<fn>{backend-to-backend}。
...
//footnote[backend-to-backend][マイクロサービス間の通信に使う流派などもありますがウチではノーカンです]
...
宣言的UI、というのは雑にまとめると"こういうデータだったらUI表示はこう決まる"という記述の集まりです。
...
宣言的UIを実現するためのフレームワークを使っているのであれば、状態にしたがって画面がレンダリングされます。
すると、アプリケーションを作る上での代表的な問題は、グローバルに1つだけ存在する状態オブジェクトを、どうやって簡単に部分的に取得・更新するか？ということになります。

大胆に言い切ってしまえば、GraphQLはこの状態データに対して効率的にビューを作り、パッチを当てるためのシステムです。
GraphQLのキャッシュは、つまりはたった1つの状態データとイコールの存在なのです。
よって、よいスキーマというのは状態データへのパッチ作業、言い換えるとサーバ上のデータを不足なく不便なくキャッシュに反映しやすいものを指します。

これは、頭でわかっていても難しいことです。
バックエンドとフロントエンドで開発者が異なる場合、バックエンド側ではフロントエンドでの便利なこと、つらいことというのには目が向きにくいものです。
